...
| `common/src/store.rs::AuthorizedStoreInfoV1::verify` at version 0; `ui/src/state.rs::on_contract_state` | Version-0 store info carries nothing: the contract refuses anything but the default, and the buyer side reads version 0 as "no details published", so an unsigned encryption key is never encrypted to (PR #82 re-review). | **Yes** -- `version_zero_info_must_be_the_default` (red with the old skip restored) and `version_zero_store_details_are_not_believed` (red with either buyer-side guard removed). The store-v0 `fdev verify-merge` corpus went from 6 violations to 0. |
...
| `common/src/store.rs::Bytes32` in the order summary | The summary's id and digest encode as CBOR byte strings, so a summary at `MAX_ORDERS` is about 284 KiB rather than 512. | **Yes** -- `the_order_summary_at_the_cap_is_byte_strings`, red with integer-array encoding. `OrderId` itself is unchanged, since it is inside every signed order. |
