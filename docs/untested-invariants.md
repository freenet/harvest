...
| `ui/src/state.rs::AppState::on_address_reuse_absent`, `on_late_address_reuse_state` | "Not found" is treated like the timeout (signs, logs, keeps listening), and a later answer showing use flags the order and tells the seller, until the node starts watching the address as an order's. | **Yes** -- `a_late_answer_showing_use_flags_an_invoice_already_signed`, mutated red by dropping the "keep listening" record on `NotFound` and by removing the cutoff at watch start (which then flags the invoice's own registration). A late answer that arrives after the watch begins is lost. |
