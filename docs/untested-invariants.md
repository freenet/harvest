...
| Where | Claim | Caught? |
|---|---|---|
| `common/src/store.rs::normalize_copies`, `verify` | A wrapped copy is kept exactly while its backer holds an unretired backing (the retirement is the custody tombstone), at most four scopes per backer, the smallest kept; a state holding a copy for a key that does not back the store, or whose backing is retired, is refused; every merge of valid states succeeds. | **Yes** -- `retiring_a_backer_tombstones_every_copy_it_had_or_will_have`, `a_cut_backing_takes_its_copies_with_it`, `a_wrapped_copy_is_the_store_keys_for_a_backer_it_holds`, `a_backer_keeps_its_smallest_scopes_past_the_bound`, `seeded_random_stores_with_copies_obey_the_merge_laws`; red with the retired filter, the backed filter and the verify check removed, and with the bound shifted and reversed. `fdev verify-merge` on `store-copies` and `store-copies-bad`: see the PR. |
...
| `WrapSecret`'s `Debug`, `delegate::WrapSignature` | The wrap signature never prints. | **Yes** -- `a_wrap_secret_never_prints_its_signature`; red with the redaction removed. Whether the browser console ever sees it depends on harvest#96 (the fix for #94), which this stack sits on. |
