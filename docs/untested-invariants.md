...
| `Complaint::verify`, exact envelopes (#143 review round 1, P1-3) | Nothing rides on a complaint that nobody signed for: the buyer's `scoped_payload` is byte for byte the envelope of its terms, and the order's is byte for byte the envelope of its order terms (`backing::is_exact_harvest_envelope`, canonical or legacy webapp id). `verify_scoped_signature` alone tolerates bytes after the CBOR item and map keys the decoder skips, a free-text channel on a permanent record. | **Yes** -- `a_complaint_envelope_carrying_extra_bytes_is_refused` (trailing text and an extra key, each re-signed, and each passing the loose check first), `a_complained_about_order_whose_envelope_carries_extra_bytes_is_refused`; red with each half removed. |
| `Complaint::verify`, a real payment (nit) | An order for zero sats, or an on-chain order counting as paid at zero confirmations, takes no complaint. | **Yes** -- `a_complaint_about_a_free_or_unconfirmed_order_is_refused`; red with either check removed. |
