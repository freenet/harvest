...
| Travelling `harvest:folded:` records | A predecessor sealed here is done for the NEXT generation too, so a deletion made here (a forgotten conversation, an unwatched address) is not brought back by the next re-key. | **Yes** -- `a_folded_predecessor_is_done_for_the_next_generation` (delegate), `a_generation_folded_into_a_newer_one_is_not_walked_again` (walk). |
...
| `gateway::delegate_migrate_ops`; `delegate_migrate::SettleGate` | One call in flight; a `Missing` error for the waited-on delegate answers at once; a predecessor's late answer never reaches `AppState`; `InitEncryptionKey` runs only after a walk that reached every registered generation, and is dropped for the load otherwise; imported secrets trigger a refresh, including `GetRsaPublicKey` for imported RSA keys only. | **Partly.** The gate is host-tested (`deferred_work_waits_for_a_complete_walk`) and the payload decision is `Expect::accepts_payload` (`the_other_expectations_match_only_their_own_answer`); the slot, timers and refresh are wasm-only and exercised by the rehearsal. |
...
| Residuals | Not recoverable: anything held only under V1-V4, or under a generation this node never registered. A stale tab of an OLD UI can write to its old delegate after that generation is sealed, and that write is not carried. A conversation forgotten on an old generation can come back from an older one under Union. | **Documented** at `delegate_migrate`'s module docs. |
