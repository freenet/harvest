...
| `ui/src/state.rs::AppState::apply_address_state` | A copy of an address contract that scanned less far than the one already held is refused, so re-reading cannot un-retract a reversed payment. | **Yes** -- `an_answer_that_scanned_less_far_is_ignored`, mutated red by removing the watermark comparison. This is the guard that keeps repeated reads able to make a settlement only LATE and never WRONG. |
