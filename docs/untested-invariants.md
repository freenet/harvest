...
| `ui/src/state.rs::AppState::settlement_publish_failed` | A settlement update that did not reach the node is published again rather than abandoned for the session, and only the SELLER attempts a publish at all. | **Yes** -- `a_settlement_that_failed_to_send_is_published_again` and `a_buyers_tab_does_not_try_to_publish`, both mutated red. The send itself is still wasm-gated; the withdrawal was moved out of the spawned task specifically so the part that decides could be tested. |
| `ui/src/state.rs::AppState::apply_address_state` | A copy of an address contract that scanned LESS FAR than the one already held is refused. | **Yes** -- `an_answer_that_scanned_less_far_is_ignored` (a stale copy does not erase claims or lower the balance) and `an_answer_at_the_same_height_still_lands` (the equal case, whose failure direction is losing a payment), both mutated red. **It narrows the un-retraction window rather than closing it:** a retraction is an extra claim added within a scan window, and the watermark only advances on a strictly greater height, so a same-height reorg leaves a copy that is admitted and lacks the retraction. Closing that needs an ordering finer than the watermark. Also a total order only while ONE bridge is trusted, which is what ships today. |
