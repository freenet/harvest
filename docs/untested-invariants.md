...
| same | Only a FRESH read can conclude "missing". A re-read the node did not answer, a mailbox this tab does not route, or a RESEND that cannot reach the node (after the first copy did) never produces "the seller has not received this"; they produce "Harvest could not check", with the same Send again. | **Yes** -- `an_unanswered_reread_never_reports_the_message_missing` (red when absence from a stale read counts, and red when this case stays silent), `a_mailbox_this_tab_cannot_see_never_reports_the_message_missing`, `a_resend_that_cannot_reach_the_node_claims_nothing_about_the_first_copy` (red when a later write failure is reported as "never reached your node"). |
...
| `ui/src/components/message_view.rs::Thread` | A message the check gave up on says the seller has not received it, with "Send again". | **No.** No component tests in this repository (see the `message_view.rs` row below). The state it renders from (`SentMessage::not_arrived`) is tested. |
