...
| `StoreStateV1::normalize_backings`, `verify`; `SignedSetV1::verify` | Every merge of valid states succeeds: past 64 backings the smallest Ghost Keys are kept, each retirement goes with its backing, the closed flag and everything else in the update always lands, and a cut backing never returns un-retired. A retirement names a held backing; every record sits in its own slot; a refused (badly signed) delta leaves the state unchanged. | **Yes** -- `a_union_past_the_bound_keeps_the_smallest_keys_and_everything_else` (red with the largest kept and with the normalisation skipped), `seeded_merges_past_the_bound_obey_the_merge_laws_and_drop_nothing_else` (24 states over 72 backings, 150 pair/triple cases, closure and listing survival asserted on every merge), `a_cut_backing_takes_its_retirement_and_never_returns_unretired`, `a_retirement_of_a_key_that_does_not_back_the_store_is_refused`, `a_record_under_someone_elses_slot_is_refused`, `a_refused_record_leaves_the_state_unchanged`. `fdev verify-merge` with corpora whose unions pass the bound, with delta steps: see the PR. **History:** the first version refused past the bound (Must Fix 1 of the PR #98 review); `fdev` counted that as inconclusive, not as a violation. |
...
| `ui/src/backing_flow.rs` single-flight and section 6.2 | A second creation or move is refused while one is under way, whatever stage it has reached; a Ghost Key that already backs a loaded store cannot back another; a failure releases the creation for a retry; no store key is minted without a block to date the backing to. | **Yes** -- `a_second_creation_is_refused_until_the_first_finishes`, `a_ghost_key_that_already_backs_a_store_cannot_back_another`, `a_refused_store_key_abandons_the_creation_and_says_so`, `with_no_block_no_creation_starts`. **Limit:** the marker lives for the session; after a reload mid-creation, the section 6.2 check stops a duplicate only once the first store has loaded. |
