...
| `delegate_migrate` policy | A generation the node never ran (`Missing`) does not hide an older one; a generation that is registered but silent STOPS the walk, so an older one cannot land first and stand; the newest generation's value wins a shared key; the current delegate's own value stands. | **Yes** -- `a_skipped_generation_does_not_hide_an_older_one` (red under `NewestSnapshotWins`), `a_silent_newer_generation_stops_the_walk_until_it_answers`, `the_newest_generations_value_wins`, `a_secret_the_current_delegate_holds_is_kept`. |
