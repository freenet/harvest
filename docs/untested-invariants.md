...
for the fixes review rounds 1 to 3 forced (see the rows marked "round 1",
"round 2" and "round 3").
...
| `fulfilment::offers_payment_address` (round 2) | The address is offered only while a payment sent now could still confirm inside the window (tip at or below the window's last block), independent of the stage, which stays "awaiting payment" past the window while a payment made in time gathers confirmations. | **Yes** -- `only_an_open_invoice_offers_its_address_and_only_inside_its_window`; red with the cutoff moved out to the stage's `settle_until`, and (round 3) with the cutoff made inclusive of the window's last block, when a payment sent confirms outside it. Past the cutoff, `closed_window_note` says why the address is gone (red with the note suppressed). |
