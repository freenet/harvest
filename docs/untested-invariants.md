...
| `fulfilment::complaint_standing`, `complaint_window_end` | A complaint signed after the window (the later of the despatch deadline and the despatch's anchor, plus `COMPLAINT_WINDOW_BLOCKS`, the same end `order_stage` reads) is shown as late and not counted. | **Yes** -- `a_complaint_counts_inside_the_window_and_not_after`; red with the late rule removed. **Stated limit:** `block_ref` is a lower bound only (every past block hash is public), so a buyer can backdate a late complaint into the window. The rule catches an honestly late complaint and nothing more. |
...
| `BuyerPurchase::paid`, `AppState::paid_order`, `complaint_checks` (P1-1) | Whether a buyer may complain is decided only on what the seller cannot change after payment: the order is `Paid`, the whole record verifies under the store contract's OWNER key, and it names this conversation's receipt key. Not on the payment blockers (`StoreClosed`, `SellerIdentityUnknown` are the seller's to cause) and not on `store_verifying_key` (`None` once the backing is retired). The card shows a paid purchase as paid, with the complaint, whatever the payment checks say. | **Yes** -- `a_seller_cannot_take_the_complaint_away_after_payment` (store closed, backing retired, both), `paid_order_requires_paid_and_the_owner_key`; red with eligibility through `settled()`, with the key from `store_verifying_key`, with the owner verify removed and with the status check removed. |
| `AppState::remember_paid_purchases`, `buyer_purchases`, delegate `paid_purchases` (P1-2) | The buyer's delegate keeps its own copy of each paid order once seen (`harvest:paid_order:{order id}`, verified `Paid` under the named store key, bounded, capped, first copy kept), and a purchase falls back to it when the store has dropped the order (4,096 free unpaid orders evict it under `enforce_order_cap`) and when the mailbox has dropped the acceptance. A copy counts only for its own store key and conversation. | **Yes** -- `a_paid_order_the_seller_evicted_is_still_complained_about` (order and despatch evicted, then the acceptance too; complaint filed and verified), `a_kept_copy_is_used_only_for_its_own_store_and_conversation`, `a_failed_paid_order_copy_is_sent_again`, and the delegate module's tests; red with the store-order fallback, the kept-copy loop, the send, and the store-key filter each removed. **Limit:** with the order evicted its despatch goes too (`normalize_fulfilment`), so the complaint opens at the despatch deadline rather than at the despatch. And the copy is made only once this node has SEEN the order paid; a seller who evicts it before the buyer's node ever loads the paid state leaves nothing to copy. |
