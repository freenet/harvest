...
### Address reuse and pre-order payments (harvest#77, 2026-09-18)
...
itself. Two mechanisms were added, and this is what each one does and does not
have behind it.

| Where | Claim | Caught? |
|---|---|---|
| `harvest_common::payment::verify_on_chain_proof` | A payment whose outpoint confirmed at or below the order's anchor block does not count toward the order, so it can neither settle it nor, through a later retraction, reverse it. Enforced by the store contract because `AuthorizedOrder::verify` calls it. | **Yes** -- `store::order_tests::a_payment_that_confirmed_before_the_order_does_not_settle_it` and four neighbours, plus the contract-level `refuses_an_order_settled_by_a_payment_older_than_the_order` (through `validate_state`) and the UI-level `a_payment_older_than_the_order_does_not_settle_it` (through `settled_orders`). Mutations run, each red: dropping the skip (five `order_tests` red, and the contract and UI tests red); `<=` to `<` (the boundary test red); judging only an outpoint's latest confirmation instead of any of them (`a_pre_order_payment_re_mined_after_the_anchor_still_predates_it` red); treating a missing anchor as height 0 (`an_on_chain_order_without_an_anchor_cannot_be_paid` red). |
| same | The rule closes the reported case, not every reuse. | **Pinned by reasoning only.** It does not catch a payment that was broadcast before the order but still unconfirmed when it was signed, nor one that confirmed inside the lag between the real tip and a stale anchor (bounded by `MAX_ANCHOR_AGE_BLOCKS` on the buyer's side). A `MempoolOutput` sighting at an `as_of` below the anchor would close the first, and was left out deliberately: `as_of` is the observing bridge's own position, so a lagging bridge would make an honest payment read as early and strand it. |
| `delegates/harvest-delegate/src/bitcoin.rs::apply_published_floor` | The counter is raised past every index of this key whose script the store has published, never lowered, and orders from another key move nothing. | **Yes** -- the five recovery tests in `bitcoin::tests` and the handler-level `a_fresh_install_does_not_reissue_a_published_address`. Mutations run, each red: removing the floor from `DeriveOrderAddress`, and separately from `SetPaymentXpub` (the handler test); scanning from 0 and assigning (`a_device_ahead_of_the_network_keeps_its_count`); not extending the window on a match (`each_match_extends_the_scan`). |
| same | It recovers the highest index ever HANDED OUT. | **No, and it cannot.** It recovers the highest index ever PUBLISHED. An index burned by an invoice abandoned before publishing, on a device that is gone, is invisible to it, as is an order pruned from the store at `MAX_ORDERS` and one more than `PUBLISHED_INDEX_GAP` past the last match (pinned as a known limit by `a_published_order_beyond_the_gap_is_not_found`). Each can be issued again; the pre-order rule above is what stops the old payment settling the new order. |
| `ui/src/state.rs::AppState::issue_invoice` | No address is derived until the store being invoiced has been read from the network. | **Yes** -- `an_invoice_waits_for_the_store_to_load`, mutated red by making the check always pass. "Read from the network" means `BrowsingStore::info` is set, which only `on_contract_state` does. It is NOT evidence the copy read is current: a node that has just re-created the store may hand back its own fresh state before the network's copy has merged in, and then there is nothing to move past. The pre-order rule covers that window. A sibling store under the same payment key that has not loaded contributes nothing either. |
| `ui/src/state.rs::AppState::published_payment_scripts` | Every order in every store this seller owns, under every Ghost Key, is sent. | **Yes** -- `the_published_scripts_are_every_owned_stores_orders`, mutated red by reading only the first Ghost Key's stores and by reading every browsed store. |
| `ui/src/state.rs::spawn_order_address_request`, `ui/src/components/invoice_form.rs::save_payment_key` | The scripts actually reach the delegate. | **No.** Both are `#[cfg(target_arch = "wasm32")]` sends: type-checked by the wasm clippy step, never executed. Passing `Vec::new()` at either call site would compile and pass every test, and restore the bug. The cost of the scan in the delegate's wasm was not measured either; natively it is about 70 microseconds per index, so `PUBLISHED_INDEX_GAP` (100) adds roughly 7 ms per invoice before the wasm slowdown. |
