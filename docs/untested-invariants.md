...
| Reader: the record's load state (P1-5) | A record that has not loaded, was not found, or failed to load is never shown as "Clean record". | See the row added with the fix in `store_view`. |
