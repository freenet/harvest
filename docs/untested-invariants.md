...
| `ui/src/gateway/mailbox_ops.rs::deliver` | A buyer's message that is not in the mailbox after the check is re-sent once, as the IDENTICAL bytes, and then reported; one that landed is not sent again. | **Yes** -- `a_message_that_never_lands_is_resent_once_then_reported` and three neighbours, red with the resend removed and with the report removed. That the resend is idempotent rests on the mailbox keying entries by `entry_digest` (tested in `dedup_tests`), and on `AppState::take_for_resend` returning the stored sealed message (`a_resend_is_the_identical_sealed_message`, red when it returns a changed nonce). |
| same | "Landed" means in the buyer's own node's copy of the mailbox, re-read after `DELIVERY_CHECK_AFTER_MS`. | **Partly.** The re-read waits for the node's answer (the same waiter as priming). Whether the network then carries the message from the buyer's node to the seller's is not observable from the buyer's side, so the check cannot see a message that applied locally and never propagated. |
