...
| `custody_flow::expire_custody`, `spawn_custody_request` (#99 re-check) | A custody request nothing answers is given up after `CUSTODY_TIMEOUT_MS` (the watch requests' bound), and one whose send fails is given up at once; either way the seller is told and the vault is free again. | **Partly** -- `an_unanswered_custody_request_times_out`. The send-failure path and the timer are wasm-only and compile-checked. |
