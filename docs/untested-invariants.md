...
original file restored after each); all 13 went red.
...
| `state.rs::AppState::cancel_invoice` | Only the store's owner, only an order the store holds, only one still awaiting payment, and not while a cancellation of it is already signing. | **Yes** -- `only_the_stores_owner_can_cancel`, `a_paid_invoice_cannot_be_cancelled`, `a_second_cancel_while_the_first_is_signing_is_refused`; each red with its guard removed. |
...
| `fulfilment::paid_height` | The despatch window runs from the block at which in-window confirmed value first reached the amount, read from the `Paid` record's own evidence via the verifier's fold, so every reader of the published record agrees; value outside the payment window is not the order's payment; only a `Paid` record has one. | **Yes** -- `the_paid_height_is_where_the_payment_first_covered_the_order`, `a_later_overpayment_does_not_move_the_paid_height`, `value_outside_the_payment_window_is_not_the_orders_payment`, `only_a_paid_record_has_a_paid_height`; red with the cumulative total replaced by any-value and with the window filter removed. **Stated limit:** claims are decoded without re-checking their signatures, which is sound only because every record reaching it was already verified (by the store contract, and by `payment_blockers` on the buyer's side). A caller that feeds it an unverified record gets an unverified date. |
| `fulfilment::order_stage` | Unpaid lapses strictly after the payment window's last block; paid is awaiting despatch through `paid + DESPATCH_WINDOW_BLOCKS` inclusive, overdue through a further `COMPLAINT_WINDOW_BLOCKS` inclusive, then closed; no window is judged without a tip or a readable paid height. | **Yes** -- `a_paid_order_moves_through_despatch_complaint_and_closed_by_the_tip_alone`, `an_unpaid_order_lapses_when_its_payment_window_closes`, `no_window_is_judged_without_a_tip_or_a_paid_height`; red with each boundary moved by one. |
