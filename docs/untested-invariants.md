...
itself. Three mechanisms were added, revised after the PR #83 review, and
...
| `ui/src/state.rs::AppState::order_address_request`, `set_payment_xpub_request` | The requests carry the published scripts: every script on setting a key, and on a derivation only those the delegate has not already answered for in this tab. | **Yes** -- `the_address_request_carries_the_published_scripts` and `a_script_the_delegate_has_answered_for_is_not_sent_again`, mutated red by sending `Vec::new()` and by dropping the accounted-for filter. The wasm sends now only call these builders; what is untested is that they are called at all, which the compiler enforces for `derive_order_address` (it has no other way to get a request) but not for a future second send site. A script marked accounted that was beyond the delegate's scan gap is not sent again in that tab. |
