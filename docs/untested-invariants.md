...
| `Certificate` arm, `on_certificate_request_failed`, expiry | Custody deferred while a listing waited is started again when the wait ends. | **Partly** -- `a_certificate_arriving_starts_custody_deferred_behind_a_listing`; red with the call removed from the `Certificate` arm. The other two call sites are not driven. |
