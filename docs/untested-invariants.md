...
| same | Only a FRESH read can conclude "missing": a re-read the node did not answer, or a mailbox this tab does not route, never produces "the seller has not received this". | **Yes** -- `an_unanswered_reread_never_reports_the_message_missing` (red when absence from a stale read counts) and `a_mailbox_this_tab_cannot_see_never_reports_the_message_missing`. |
