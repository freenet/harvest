...
description's re-key table (at review round 5: reputation `eb3e0e67`, store
`34099f2b`, mailbox `4b02a3f4`, index `0a21fecf`, delegate `4ffe901a`), and
...
| `ReputationStateV1::apply_delta` | All or nothing; two complaints for one order resolve to the same survivor (the smaller encoding) whichever arrives first; the state is strictly ascending by order id. | **Yes** -- `a_delta_holding_one_invalid_complaint_applies_none_of_it`, `two_complaints_for_one_order_converge`, `apply_delta_and_merge_normalise_the_state_they_are_applied_to`, `seeded_random_reputations_obey_the_merge_laws`; red with the tie-break made first-writer-wins and with strictly-ascending relaxed. |
