...
| `ui/src/state.rs::AppState::order_address_request`, `set_payment_xpub_request` | The requests carry the published scripts: every script on setting a key, and on a derivation those the delegate has not MATCHED, with unmatched ones offered again once the counter has advanced by half of `PUBLISHED_INDEX_GAP`. Setting the key waits for every owned store to load, and the record is cleared when the delegate reports a lower count or another key. | **Yes** -- `the_address_request_carries_the_published_scripts`, `only_scripts_the_delegate_matched_are_not_sent_again`, `a_payment_key_waits_for_every_owned_store_to_load`, and the delegate's `the_delegate_reports_only_the_scripts_it_matched`. Mutated red: sending `Vec::new()`; marking every sent script accounted; no load gate; no reset on a falling count; the delegate reporting everything it was sent as matched. |
| `ui/src/state.rs::AppState::orders_sharing_an_address`, `settled_orders` | Two of the seller's own orders on one address with overlapping windows are both flagged on the card, and neither is auto-published as Paid. Local and writes nothing, so it is merge-safe; it is the backstop for the two-device race and every lookup miss. | **Yes** -- `orders_share_an_address_only_when_their_windows_overlap` (both window boundaries, and a stranger's store flagging nothing) and `a_payment_on_an_address_two_own_orders_share_is_not_auto_published`. Mutated red: settling regardless; strict overlap comparison; counting every browsed store. **Limits:** only orders in loaded stores are compared, so a twin in a store that has not loaded is invisible; and nothing publishes Paid for a flagged order afterwards, so the seller settles those by hand. The card wiring (`OrderCard` reading `address_warning`) is a component and is not tested. |
| `ui/src/state.rs::AppState::on_address_reuse_absent`, `on_late_address_reuse_state` | "Not found" is treated like the timeout (signs, logs, keeps listening), and a later answer showing use flags the order and tells the seller, until the node starts watching the address as an order's. | **Yes** -- `a_late_answer_showing_use_flags_an_invoice_already_signed`, mutated red by dropping the "keep listening" record on `NotFound` and by removing the cutoff at watch start (which then flags the invoice's own registration). A late answer that arrives after the watch begins is lost. **Measured once, 2026-09-18**, on a throwaway peer (`freenet network`, WS port 7661, connected to the nova gateways) with `fdev execute get` on eight random never-existing instance ids: seven answered `NotFound` in 4.3-7.1 s, one took 80.8 s. So the 15 s wait usually ends on `NotFound` rather than the timeout, and occasionally a fresh invoice waits the full 15 s. One peer, one sitting, eight samples: a spot check, not a distribution. |
