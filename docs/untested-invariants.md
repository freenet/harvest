...
| `delegate_migrate` policy | A generation the node never ran (`Missing`) does not hide an older one; the newest generation's value wins a shared key; the current delegate's own value stands. | **Yes** -- `a_skipped_generation_does_not_hide_an_older_one` (red under `NewestSnapshotWins`), `the_newest_generations_value_wins`, `a_secret_the_current_delegate_holds_is_kept`. |
...
| `import::import_secret` | Standalone secrets are never overwritten; a store registry, the transaction index and the watch list are merged by their handlers' identities; a capped family is not overfilled and a full one is retryable; a refused write is retryable; nothing outside `harvest:` and no store key is written; only the Harvest web app may import. | **Yes** -- the `import::tests` module, each named mutation red. |
...
| `gateway::delegate_migrate_ops` | One call in flight; a `Missing` error for the waited-on delegate answers at once; a predecessor's late answer never reaches `AppState`; `InitEncryptionKey` waits for the walk (or 30 s) so a new key is not minted ahead of the import of the old one; imported secrets trigger a refresh of what the app read on connect. | **Rehearsed, not unit-tested** (wasm-gated): see below. |
