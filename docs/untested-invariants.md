...
| `Certificate` and `GhostKeyDetail` arms, `on_certificate_request_failed`, expiry, `watch_signature_failed` | Custody deferred while a listing (or a watch request) waited is started again when the wait ends. | **Partly** -- `a_certificate_arriving_starts_custody_deferred_behind_a_listing`; red with the call removed from the `Certificate` arm. The other call sites are not driven. |
