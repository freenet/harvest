...
| `ui/src/state.rs::AppState::refresh_shared_address_warnings`, `settled_orders` | Two of the seller's own loaded orders on one address and network with overlapping windows are flagged. One awaiting payment is not auto-published as Paid while the confirmed value inside its own window cannot cover it plus every overlapping twin still awaiting payment, and is released once it can; one already paid is only warned about. The pill never uses the paid style while a warning shows. Recomputed on store, address and store-list changes, not per render. | **Yes** -- `shared_address_warnings_hold_until_the_address_covers_the_twins` (both window boundaries, hold and release, a paid twin, another network, a stranger's store) and `a_payment_on_an_address_two_own_orders_share_is_not_auto_published` (held, then released through `settled_orders`). **Limits, stated plainly:** it exists only in the seller's own UI, so it holds only because only the seller's tab publishes Paid today; #75 (letting a buyer's tab publish) would bypass it. A twin in a store that has not loaded is not seen. The card wiring is a component and is not tested. |
