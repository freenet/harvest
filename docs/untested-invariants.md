...
| `SignedSetV1::verify` / `apply_delta` | A set holds at most its cap (64 backings, 64 retirements, one closure), every record sits in its own slot, and a refused delta leaves the state unchanged. | **Yes** -- `a_store_past_the_backing_cap_is_refused` (red with either cap removed; the `apply_delta` half survived at first, because the test's helper ran `verify` after it, and now calls `apply_delta` alone), `a_record_under_someone_elses_slot_is_refused`, `a_refused_record_leaves_the_state_unchanged`. |
...
| Merge beyond the cap | The merge is total. | **No, by design.** Past 64 backings or retirements the merge refuses rather than dropping an entry, since dropping would break "a backing is never removed". Only the store key can add either, so only the seller can reach it, by attaching more than 64 Ghost Keys on two replicas before they converge. |
...
| Store keys across a delegate re-key | A store key survives the delegate being rebuilt. | **No, and not true.** Nothing drives the export handshake, so a delegate re-key loses every store key; the store stays readable and cannot be signed for. Phase 1b's custody (the key wrapped to each backing Ghost Key in store state) closes it. |
...
| `ui/src/state.rs::payment_blockers`, `components/store_view.rs::buyable` | A closed store is refused before anything else and offers no Buy control; a commitment is checked against the store key, not the Ghost Key. | **Yes** -- `a_closed_store_is_refused_before_anything_else`, `a_closed_store_cannot_be_bought_from`, `a_commitment_is_checked_against_the_store_key_not_the_ghost_key`; each red with its check removed or pointed at the Ghost Key. The closed banner is a component and is not tested. |
| `ui/src/state.rs::store_owner_key`, `merge_store_registrations` | The owner an update names is the registration's store key, never the Ghost Key, and a store list answer never takes a known store key away. | **Yes** -- `the_owner_of_our_store_is_the_key_every_update_names`, `a_store_list_answer_keeps_a_locally_known_store_key`; red with a fallback key substituted and with the carry-over removed. |
