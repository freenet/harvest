...
fn a_new_webapp_id_is_a_new_wrapping_key() {
    ...
    assert_ne!(old.copy_id(), new.copy_id());
}
...
// --- Check 4: the custody field and its merge -----------------------------

fn copy_id(backer: u8, scope: u8) -> CopyId {
    CopyId {
...
fn copy(tag: u8) -> WrappedStoreKey {
    WrappedStoreKey {
        scheme: SCHEME_V1,
        ciphertext: vec![tag; WRAPPED_LEN_V1],
    }
}

fn merged(a: &StoreKeyCustody, b: &StoreKeyCustody) -> StoreKeyCustody {
    let mut out = a.clone();
    out.merge(b);
    out
}
...
#[test]
fn retiring_a_backer_tombstones_every_copy_it_had() {
    let mut s = StoreKeyCustody::default();
    s.add_copy(copy_id(1, 7), copy(1)).unwrap();
    s.add_copy(copy_id(1, 8), copy(2)).unwrap();
    s.add_copy(copy_id(2, 7), copy(3)).unwrap();
    let stale = s.clone();

    s.retire(&SigningKey::from_bytes(&[1; 32]).verifying_key());
    assert_eq!(s.copies.len(), 1);
    assert!(s.check().is_ok());

    // A stale peer's copy merges back to the tombstone, in either order.
    assert_eq!(merged(&s, &stale), s);
    assert_eq!(merged(&stale, &s), s);

    // And a fresh copy for the retired key, even under a new scope, is refused.
    assert_eq!(
        s.add_copy(copy_id(1, 9), copy(4)).unwrap_err(),
        CustodyError::Retired
    );
}

#[test]
fn an_unnormalised_or_oversized_or_malformed_state_is_refused() {
    let mut s = StoreKeyCustody::default();
    s.copies.insert(copy_id(1, 7), copy(1));
    s.retired.insert(copy_id(1, 7).backer);
    assert!(s.check().is_err(), "a ciphertext beside its own tombstone");
    ...
    let mut s = StoreKeyCustody::default();
    for i in 0..MAX_WRAPPED_COPIES {
        s.add_copy(copy_id(1, i as u8), copy(0)).unwrap();
    }
    assert_eq!(
        s.add_copy(copy_id(2, 0), copy(0)).unwrap_err(),
        CustodyError::TooManyCopies
    );
}

#[test]
fn the_custody_field_round_trips_canonically() {
    let mut s = StoreKeyCustody::default();
    s.add_copy(copy_id(1, 7), copy(1)).unwrap();
    s.add_copy(copy_id(2, 8), copy(2)).unwrap();
    s.retire(&SigningKey::from_bytes(&[3; 32]).verifying_key());
    let bytes = crate::to_cbor(&s).unwrap();
    let back: StoreKeyCustody = crate::from_cbor(&bytes).unwrap();
    assert_eq!(back, s);
    assert!(crate::is_canonical_cbor(&back, &bytes));
    // The ciphertext is a CBOR byte string (0x58 0x30 = bytes, length 48).
    let needle = [0x58, 0x30, 1, 1];
    assert!(bytes.windows(4).any(|w| w == needle));
}

/// Seeded random merge laws, byte for byte, over states that mix live
/// copies, conflicting copies for one id, and tombstones.
#[test]
fn seeded_random_custody_states_obey_the_merge_laws() {
    let ids: Vec<CopyId> = (1..=4u8)
        .flat_map(|b| (7..=8u8).map(move |s| copy_id(b, s)))
        .collect();
    let copies: Vec<WrappedStoreKey> = (0..3u8).map(copy).collect();
    let backers: Vec<[u8; 32]> = (1..=4u8).map(|b| copy_id(b, 0).backer).collect();

    let mut rng = Rng::new(0xc057_0d1e);
    let states: Vec<StoreKeyCustody> = (0..200)
        .map(|_| {
            let mut s = StoreKeyCustody::default();
            for r in rng.subset(&backers, 2) {
                s.retired.insert(r);
            }
            for id in rng.subset(&ids, 5) {
                let c = copies[rng.below(copies.len())].clone();
                s.copies.insert(id, c);
            }
            s.normalize();
            assert!(s.check().is_ok());
            s
        ...
        .collect();
    assert_laws(&states, 500, &mut rng, merged, |s| {
        crate::to_cbor(s).unwrap()
    });
    // Merge is also inflationary: nothing retired is ever un-retired.
    for (i, a) in states.iter().enumerate() {
        let b = &states[(i * 7 + 3) % states.len()];
        let m = merged(a, b);
        assert!(a.retired.is_subset(&m.retired) && b.retired.is_subset(&m.retired));
