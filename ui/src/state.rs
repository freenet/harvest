...
pub struct AppState {
    ...
    /// Invoices signed without a definite answer from the reuse check (the
    /// node said "not found", or the wait timed out), keyed by the address
    /// contract, so a late answer showing the address WAS used can still
    /// reach the seller. Bounded by [`MAX_UNCHECKED_ADDRESSES_KEPT`].
    pub signed_unchecked: std::collections::VecDeque<(Vec<u8>, harvest_common::payment::OrderId)>,
    /// Orders whose address a late answer showed had been used before.
    pub orders_on_used_addresses: HashSet<harvest_common::payment::OrderId>,
...
/// What the watch-request loop should do now.
#[derive(Debug, Default, PartialEq)]
pub struct WatchWork {
    /// Fetch, and subscribe to, the inbox.
    pub fetch: Option<freenet_stdlib::prelude::ContractInstanceId>,
    /// Requests queued for the delegate's signature.
    pub sign: Vec<crate::bitcoin_inbox::PendingInboxEntry>,
}
...
pub fn order_for_invoice(
    ...
    conversation_keys: Option<&crate::messaging::ConversationKeys>,
) -> Result<harvest_common::payment::Order, String> {
    use harvest_common::payment::Order;

    // Which listing this answers, tagged so only the buyer and seller can
    // tell (harvest#57). An invoice answering a request must carry it: the
    // buyer refuses an untagged order for a conversation that asked for
    // something, and the seller recognises an answered request by it.
    let listing_tag =
        match (pending.reply_to, conversation_keys) {
            (None, _) => None,
            (Some(_), Some(keys)) => Some(keys.listing_tag(&pending.listing_id)),
            (Some(_), None) => return Err(
                "this invoice answers a buyer's request, but your delegate has not produced that \
                 conversation's key, so the order could not say which listing it is for"
                    .to_string(),
            ),
        };
...
/// Why the seller's card warns about an order's payment address.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum AddressWarning {
    /// Another of this seller's orders uses the same address and their
    /// payment windows overlap, so one payment could prove both.
    SharedWithAnotherOrder,
    /// The address turned out to have been used before, after the invoice
    /// had already been signed without knowing (a late answer to the reuse
    /// check).
    UsedBefore,
}
...
impl AddressWarning {
    pub fn explain(self) -> &'static str {
        match self {
            AddressWarning::SharedWithAnotherOrder => {
                "Another of your invoices uses this same payment address, and a single \
                 payment could look like it pays for both. Harvest will not mark either as \
                 paid by itself. Do not ship either against a single payment: check your \
                 wallet for two separate payments, or reissue one of them."
            }
            AddressWarning::UsedBefore => {
                "This invoice's payment address turned out to have been used before. A \
                 payment already at it is not this invoice's. Check your wallet before \
                 shipping, or reissue the invoice, which gets a new address."
            }
        }
    }
}
...
/// How many invoices signed without a definite reuse answer are remembered,
/// so a late answer can still flag them. Oldest forgotten first.
pub const MAX_UNCHECKED_ADDRESSES_KEPT: usize = 64;
...
pub enum PaymentBlocker {
    ...
    /// The order's listing tag matches nothing this conversation asked for,
    /// or it carries none.
    ///
    ...
    ///
    /// The order publishes a tag rather than the listing id (harvest#57,
    /// `harvest_common::mailbox::listing_tag`), which this conversation can
    /// recompute for each listing it asked about. A missing tag is refused,
    /// since a seller answering a request always writes one.
    ///
    /// Does not fire when the buyer has asked for nothing in this conversation
    /// (one that began as an ordinary question): there is then nothing to
...
impl AppState {
            ...
            match freenet_bitcoin_common::from_cbor::<freenet_bitcoin_inbox::InboxStateV1>(
                ...
                Ok(inbox_state) => {
                    ...
                    if let Some(inbox) = self.bitcoin.inbox.as_mut() {
                        inbox.on_state(inbox_state, now_ms);
                    }
    ...
    pub fn settled_orders(
        ...
        let shared = self.orders_sharing_an_address();
        let mut settled = Vec::new();
        for order in &store.orders {
            if order.status != OrderStatus::AwaitingPayment {
                continue;
            }
            // Two of this seller's orders on one address with overlapping
            // windows can both be proven by ONE payment (the contract's
            // pinned known limit). Publishing Paid for either would let one
            // buyer's money read as paying for both, so neither is
            // auto-published; the card says why (PR #83 round 2, Should
            // Fix 1).
            if shared.contains(&order.order.id) {
                continue;
            }
            let Some(view) = order
                .order
    ...
    /// Orders in this seller's own loaded stores that share a payment script
    /// with another such order whose payment window overlaps theirs.
    ///
    /// # Why this exists on top of every lookup
    ///
    /// The store contract's window stops a payment settling an order made
    /// after it, or one anchored more than a window before it. It cannot stop
    /// ONE payment settling two orders on one address whose windows overlap:
    /// a cross-order rule is not merge-safe, since a `Paid` record cannot be
    /// demoted. And no lookup before signing can rule the case out: two
    /// devices invoicing at the same moment both find no contract for the
    /// address, and a lookup that times out or is wrongly told "not found"
    /// signs anyway. What is always possible is noticing afterwards, locally,
    /// from the orders themselves. That is merge-safe because nothing is
    /// written: the seller's card is flagged, and `settled_orders` declines
    /// to publish `Paid` for either order (PR #83 round 2, Should Fix 1).
    ///
    /// Own stores only. Scripts are public, so a stranger's store could copy
    /// one of this seller's; letting that suppress this seller's own
    /// settlements would hand anyone a way to stall them.
    ///
    /// An order with no anchor has no window and cannot be settled, so it is
    /// not counted as overlapping anything.
    pub fn orders_sharing_an_address(&self) -> HashSet<harvest_common::payment::OrderId> {
        let mut by_script: HashMap<&[u8], Vec<&harvest_common::payment::Order>> = HashMap::new();
        for registration in self.my_stores.values().flatten() {
            ...
            for record in &store.orders {
                if !record.order.payment_script_pubkey.is_empty() {
                    by_script
                        .entry(record.order.payment_script_pubkey.as_slice())
                        .or_default()
                        .push(&record.order);
                }
            }
        ...
        for orders in by_script.values() {
            for (i, a) in orders.iter().enumerate() {
                for b in &orders[i + 1..] {
                    if a.id == b.id {
                        // The same order listed twice (two registrations
                        // naming one store) is not two orders.
                        continue;
                    }
                    let (Some(wa), Some(wb)) = (a.payment_window(), b.payment_window()) else {
                        continue;
                    };
                    if wa.start().max(wb.start()) <= wa.end().min(wb.end()) {
                        shared.insert(a.id.clone());
                        shared.insert(b.id.clone());
                    }
                }
            }
        }
    ...
    /// What the seller's card should say about this order's address, if
    /// anything beyond what the chain shows.
    pub fn address_warning(
        &self,
        order_id: &harvest_common::payment::OrderId,
    ) -> Option<AddressWarning> {
        if self.orders_sharing_an_address().contains(order_id) {
            return Some(AddressWarning::SharedWithAnotherOrder);
        }
        self.orders_on_used_addresses
            .contains(order_id)
            .then_some(AddressWarning::UsedBefore)
    }
    ...
    /// this tab is showing, and which of them are due now (#67).
    ///
    /// Takes `&self` so the periodic tick can decide under a READ of the
    /// state. Taking it for writing re-renders every component in the app,
    /// and with the wait widened to minutes most ticks have nothing to send,
    /// so a tick that wrote first would repaint a buyer's screen once a
    /// minute for the life of an unpaid order. The walk is also not cheap
    /// (it decrypts conversations and verifies a signature per purchase), so
    /// it is done once here and its answer handed to
    /// [`Self::send_address_rereads`] rather than computed twice.
    pub fn due_address_rereads(&self, now_ms: u64) -> (Vec<[u8; 32]>, Vec<[u8; 32]>) {
        let mut wanted: Vec<[u8; 32]> = Vec::new();
        for store_contract_id in self.browsing_stores.keys() {
            for id in self.address_contracts_to_reread(store_contract_id) {
                // Defensive rather than a live guard: an address instance id
                // derives from the code hash and the order's own script, so
                // two stores naming one id is not a case anybody has
                // constructed.
    ...
    /// Send the GETs for `due`, recording each as asked for.
    ///
    ...
    /// depend on that having happened, which is why the network comes from
    /// the order itself rather than from the build's default.
    ///
    /// `wanted` is what prunes the tracker, so it must be the full set and
    /// not just `due`.
    pub fn send_address_rereads(&mut self, wanted: &[[u8; 32]], due: &[[u8; 32]], now_ms: u64) {
        ...
        for id in due {
            let network = self
                .address_network_for(id)
                .unwrap_or_else(crate::gateway::bitcoin_config::default_network);
            self.bitcoin
                .address_contract_network
                .entry(id.to_vec())
                .or_insert(network);
        ...
        for id in due.iter().copied() {
            wasm_bindgen_futures::spawn_local(async move {
                ...
                // Read WITHOUT subscribing. The subscription was made when
                // the order first appeared and the node dedups a repeat, but
                // re-subscribing on a timer asks the node to register this
                // client's interest again every few minutes, and that
                // registration is documented in core as non-idempotent. A
                // re-read needs the answer, not another subscription.
    ...
    ///
    /// The id alone does not say which network it belongs to, and the map it
    /// is registered in decides how an arriving state is read. Today every
    /// settleable order is on one network so this and the build default
    ...
    fn payment_blockers(
        ...
        if !requested.is_empty() {
            // The published order's tag says which listing it is, to this
            // conversation only (`harvest_common::mailbox::listing_tag`). A
            // missing tag on an order for a conversation that asked for
            // something is refused, not skipped: an honest seller answering a
            // request always writes one.
            let for_a_request = commitment.order.listing_tag.is_some_and(|tag| {
                requested
                    .iter()
                    .any(|listing| conversation.listing_tag(listing) == tag)
            });
            if !for_a_request {
    ...
    fn complete_invoice(&mut self, request_id: u64, derived: harvest_common::DerivedAddress) {
        ...
        let conversation_keys = invoice
            .reply_to
            .and_then(|tag| self.conversation_keys.get(tag.as_slice()));
    ...
    pub fn on_address_reuse_state(&mut self, contract_id: &[u8], state_bytes: &[u8]) -> bool {
        let Some(check) = self.address_reuse_checks.remove(contract_id) else {
            return self.on_late_address_reuse_state(contract_id, state_bytes);
        };
    ...
    /// An answer about an address whose invoice was already signed without
    /// one. If it shows the address was used, the seller is told and the
    /// order flagged; the invoice cannot be unsigned, and may already be in
    /// front of a buyer (PR #83 round 2, Should Fix 4).
    fn on_late_address_reuse_state(&mut self, contract_id: &[u8], state_bytes: &[u8]) -> bool {
        let Some(position) = self
            .signed_unchecked
            .iter()
            .position(|(id, _)| id.as_slice() == contract_id)
        else {
            return false;
        };
        let Some((_, order_id)) = self.signed_unchecked.remove(position) else {
            return false;
        };
        // Once this node watches the address as an order's, its states come
        // from that subscription -- and soon include the bridge's claims for
        // THIS invoice's own registration, which would read as "used before".
        // The late answer only means something before then, so listening
        // stops at the moment the watch begins. A late answer that arrives
        // after it is lost, which is the fail-open direction already chosen.
        if self
            .bitcoin
            .address_contract_network
            .contains_key(contract_id)
        ...
        if address_state_shows_use(state_bytes) {
            warn!(
                "A late answer shows the address of order {} was used before",
                order_id.short()
            );
            self.orders_on_used_addresses.insert(order_id.clone());
            self.notifications.push(format!(
                "Invoice {} was issued on a payment address that turned out to have been used \
                 before. A payment already at it is not this invoice's: check your wallet \
                 before shipping, or reissue the invoice.",
                order_id.short()
            ));
        }
        true
    ...
    /// Remember an invoice signed without a definite answer, so a late one
    /// can still reach the seller.
    fn note_signed_unchecked(
        &mut self,
        contract_id: &[u8],
        order_id: harvest_common::payment::OrderId,
    ) {
        if self.signed_unchecked.len() >= MAX_UNCHECKED_ADDRESSES_KEPT {
            self.signed_unchecked.pop_front();
        }
        self.signed_unchecked
            .push_back((contract_id.to_vec(), order_id));
    }

    /// The node answered that nothing is stored for this contract.
    ///
    /// Treated like the timeout, not as proof: on Freenet a GET that
    /// dead-ends can report `NotFound` for a contract that exists (PR #83
    /// round 2, Should Fix 4). So it signs, fails open for the reasons on
    /// [`Self::on_address_reuse_timeout`], logs, and keeps listening in case
    /// a real answer arrives later.
    pub fn on_address_reuse_absent(&mut self, contract_id: &[u8]) {
        if let Some(check) = self.address_reuse_checks.remove(contract_id) {
            warn!(
                "The node found nothing for address {}; issuing the invoice, and will still \
                 flag it if a later answer shows it was used",
                check.order.payment_address
            );
            self.note_signed_unchecked(contract_id, check.order.id.clone());
            self.sign_checked_order(check);
        }
    }
    ...
    /// different generation replaces the old one, along with what was sent to
    /// it: requests in an inbox the bridge no longer reads were never read, and
    /// counting them would hold the Ghost Key's places for ever.
    ///
    /// Nothing is fetched here. [`Self::queue_due_watch_requests`] fetches the
    /// inbox only once this node has an order to watch, so a buyer's tab never
    /// subscribes to it.
    ...
    pub fn withdraw_inbox(&mut self) {
        if self.bitcoin.inbox.is_some() {
            ...
            self.notifications.push(
                "The Bitcoin bridge has withdrawn its request inbox, so Harvest can no longer \
                 ask it to watch your invoices' payment addresses."
                    .to_string(),
            );
        }
        self.retire_inbox();
    ...
    pub fn watches_wanted(
        ...
        // did to explain it. A key listed once and revoked since is still
        // listed here, which is what `watch_requests_stopped` is for.
    ...
    ///
    /// Extracted from [`Self::watches_wanted`], whose filter this was, so the
    ...
    /// not the one that loses a payment. It does mean this is not a bound
    /// until a tip is known -- see `crate::address_reread`, which says so
    /// where a reader of the re-read loop will meet it.
    ...
    /// Decide what the watch-request loop does now, queueing for signature
    /// every request that is due.
    ///
    /// In order:
    ///
    /// * A request that has waited on the delegate past
    ///   [`crate::bitcoin_inbox::SIGNATURE_TIMEOUT_MS`] is given up, and its
    ...
    ///   way, for up to [`crate::bitcoin_inbox::MAX_HOLD_FOR_USER_MS`] at a
    ///   stretch. Most of the delegate's refusals name no request, so one
    ///   arriving while both are outstanding cannot be told apart. The limit is
    ///   there because some of the seller's pending states never clear, and
    ///   watching must not stop for the session behind one.
    /// * Nothing is queued while a watch request is still waiting on the
    ///   delegate, and requests are queued for one Ghost Key at a time. So a
    ///   refusal that names no key is always about exactly one.
    pub fn queue_due_watch_requests(&mut self, now_ms: u64) -> WatchWork {
        ...
        // Continuous, so separate actions of the seller's do not add up to the
        // limit.
    ...
    /// session, and tell the seller once.
    ///
    /// Stopped rather than retried later, because every way this is reached
    /// points at something a retry would make worse or not fix. A refusal, or a
    /// vault that does not answer, most likely means the grant was revoked
    /// while this tab was open: the key is still listed here, and asking again
    /// would put the vault's permission dialog in front of the seller for
    /// something they did not start, again and again, and approving it to make
    /// it stop would quietly undo their revocation. A signature by the wrong
    /// key, or one that cannot be used, will be the same next time. Reloading
    /// the page lists the keys afresh and starts over.
    fn stop_watch_requests_for(&mut self, fingerprint: &str, reason: &str) {
        if self
            .bitcoin
            .watch_requests_stopped
            .insert(fingerprint.to_string())
        {
            self.notifications.push(format!(
                "Harvest could not ask the Bitcoin bridge to watch the payment addresses of \
                 invoices issued under {fingerprint} ({reason}), and will not ask again until \
                 this page is reloaded. Payments to those invoices may not be seen until then."
            ));
        }
    }

    /// [`Self::queue_due_watch_requests`] at the current time, carried out:
    /// the inbox fetched if due, and each queued request sent for signature.
    pub fn send_due_watch_requests(&mut self) {
        let work = self.queue_due_watch_requests(now_ms());
        #[cfg(target_arch = "wasm32")]
        {
            if let Some(id) = work.fetch {
                wasm_bindgen_futures::spawn_local(async move {
                    // A failure needs no handling here: with no state arriving,
                    // the fetch is due again shortly.
                    if let Err(e) = crate::gateway::get_contract(&id, true).await {
                        dioxus::logger::tracing::warn!("could not fetch the bridge inbox: {e}");
                    }
                });
            }
            for pending in work.sign {
    ...
    /// and return the bytes to submit to the inbox. `None` if there is nothing
    /// worth sending.
    fn on_inbox_entry_signed(
        ...
        // The request is sealed to the bridge and bound to the store's seller
        // key. Signed by any other key, the inbox would admit it and the bridge
        // could not open it.
        if entry.entry.ghostkey != pending.ghostkey {
            ...
            self.stop_watch_requests_for(
                &pending.fingerprint,
                "the Ghost Key is not the one this store was created with",
            );
            return None;
        }
        // Prepared for an inbox this tab has since stopped using: sealed to it
        // and dated against its floor, so there is nowhere useful to send it.
        // Whatever it asked for is due again against the current inbox.
        let Some(inbox) = self
            .bitcoin
            .inbox
            .as_mut()
            .filter(|inbox| inbox.contract_key == pending.contract_key)
        else {
            info!("a watch request was signed for an inbox no longer in use; not sending it");
            return None;
        };
...
///
/// The route that does exist is the bridge's request inbox, a contract the
/// page writes to over the node. An invoice's payment address is sent that way
/// (`AppState::queue_due_watch_requests`), but a manual watch is not: see
/// `docs/bitcoin-integration-status.md` for why that is a separate decision.
///
...
pub struct BitcoinState {
    ...
    /// Fingerprints this tab has stopped asking to sign watch requests, for
    /// the rest of the session, after a refusal, a signature that never came
...
mod tests {
    ...
    /// **A store's state arriving runs the watch-request loop.** Observed
    /// through the one thing the loop does before it needs a verified seller
    /// key: giving up a signature request that has waited too long.
    #[test]
    fn store_state_arriving_runs_the_watch_request_loop() {
        let mut state = AppState::default();
        state
            .pending_signatures
            .push_back(PendingSignature::InboxEntry(Box::new(
                crate::bitcoin_inbox::PendingInboxEntry {
                    fingerprint: "seller-fp".into(),
                    ghostkey: freenet_bitcoin_inbox::GhostkeyId([1u8; 32]),
                    contract_key: crate::bitcoin_inbox::test_support::inbox_key(),
                    floor: freenet_bitcoin_inbox::SignedFloor::sign(
                        &crate::bitcoin_inbox::test_support::bridge_key(),
                        crate::bitcoin_inbox::test_support::FLOOR,
                    ),
        ...
        assert!(
            ...
            "the stale request was given up, so the loop ran"
        );
    }
...
mod invoice_tests {
    ...
    /// **Round 2, Should Fix 1: which pairs overlap.** Windows are
    /// `(anchor, anchor + PAYMENT_WINDOW_BLOCKS]`; two orders on one address
    /// overlap exactly when the later anchor is less than a window after the
    /// earlier. A stranger's store holding the same script flags nothing.
    #[test]
    fn orders_share_an_address_only_when_their_windows_overlap() {
        use harvest_common::payment::PAYMENT_WINDOW_BLOCKS;
        fn anchored(
            script: &[u8],
            height: u32,
            amount: u64,
        ) -> harvest_common::payment::AuthorizedOrder {
            let mut record = with_script(script);
            record.order.anchor = Some(anchor(height));
            record.order.amount_sats = amount;
            record.order = record.order.with_derived_id();
            record
        }
        let script = [0x00, 0x14, 1];
        for (later, overlaps) in [
            (100 + PAYMENT_WINDOW_BLOCKS - 1, true),
            (100 + PAYMENT_WINDOW_BLOCKS, false),
        ] {
            let mut state = seller_with_a_store();
            let a = anchored(&script, 100, 1);
            let b = anchored(&script, later, 2);
            state
                .browsing_stores
                .get_mut(STORE_ID.as_slice())
                .expect("store")
                .orders = vec![a.clone(), b.clone()];
            let shared = state.orders_sharing_an_address();
            assert_eq!(
                shared.contains(&a.order.id),
                overlaps,
                "later anchor {later}"
            );
            assert_eq!(
                shared.contains(&b.order.id),
                overlaps,
                "later anchor {later}"
            );
        }

        // A stranger's store with the same script.
        let mut state = seller_with_a_store();
        let mine = anchored(&script, 100, 1);
        state
            .browsing_stores
            .get_mut(STORE_ID.as_slice())
            .expect("store")
            .orders = vec![mine.clone()];
        state
            .browsing_stores
            .entry(vec![0x77; 32])
            .or_default()
            .orders = vec![anchored(&script, 120, 9)];
        assert!(state.orders_sharing_an_address().is_empty());
    }

    /// **Round 2, Should Fix 4.** An invoice signed on "not found" or on a
    /// timeout is still flagged, and the seller told, when an answer showing
    /// the address WAS used arrives later. Once the node watches the address
    /// as an order's, it stops listening, because the bridge's claims for
    /// this invoice's own registration would read as "used before".
    #[test]
    fn a_late_answer_showing_use_flags_an_invoice_already_signed() {
        for settle in [
            |s: &mut AppState, id: &[u8]| s.on_address_reuse_absent(id),
            |s: &mut AppState, id: &[u8]| s.on_address_reuse_timeout(id),
        ] {
            let mut state = seller_with_a_store();
            let id = invoice_awaiting_reuse_check(&mut state, 0);
            settle(&mut state, &id);
            let order_id = queued_order(&state).id;
            assert_eq!(state.address_warning(&order_id), None);

            state.on_contract_state(id, a_used_address_state());
            assert_eq!(
                state.address_warning(&order_id),
                Some(AddressWarning::UsedBefore)
            );
            assert!(state
                .notifications
                .iter()
                .any(|n| n.contains("turned out to have been used before")));
        }

        let mut state = seller_with_a_store();
        let id = invoice_awaiting_reuse_check(&mut state, 0);
        state.on_address_reuse_timeout(&id);
        let order_id = queued_order(&state).id;
        state
            .bitcoin
            .address_contract_network
            .insert(id.clone(), BitcoinNetwork::Signet);
        state.on_contract_state(id, a_used_address_state());
        assert_eq!(
            state.address_warning(&order_id),
            None,
            "the invoice's own registration read as reuse"
        );
    }
    ...
    /// **An invoice answering a request tags its listing so only the two
    /// parties can tell, and publishes no listing id.** (harvest#57)
    #[test]
    fn an_invoice_answering_a_request_tags_its_listing_privately() {
        let keys = crate::messaging::ConversationKeys::from_shared_secret(&[6u8; 32]);
        let mut answering = invoice();
        answering.reply_to = Some([0xcd; 32]);
        ...
        .expect("builds");
        assert_eq!(order.listing_tag, Some(keys.listing_tag(&listing_id())));
        let published = harvest_common::to_cbor(&order).expect("encodes");
        assert!(
            !published
                .windows(listing_id().0.len())
                .any(|w| w == listing_id().0.as_slice()),
            "the listing id itself appears nowhere in the published bytes"
        );
        ...
        assert!(
            ...
            .is_err(),
            "an invoice for a request cannot be built without the conversation's key"
        );

        let unprompted = order_for_invoice(
            &invoice(),
            &derived(0),
            Some(anchor(800_000)),
            chrono::Utc::now(),
        ...
        .expect("builds");
        assert_eq!(
            unprompted.listing_tag, None,
            "an unprompted invoice answers no request"
        );
    }
...
mod buy_flow_tests {
    ...
    /// **PR #83 round 2, Should Fix 1.** When this seller owns the store and
    /// holds a second order on the same address whose window overlaps, the
    /// confirmed payment is NOT auto-published as Paid: one payment could
    /// prove both. The same state without the twin still settles, so the
    /// refusal is the twin's doing and not the registration's.
    #[test]
    fn a_payment_on_an_address_two_own_orders_share_is_not_auto_published() {
        let (order, claims, tip) = a_paid_order();
        let (mut state, _) = buyer_after_acceptance(&order);
        give_the_node_the_chain(&mut state, &order, claims, tip);
        state.my_stores.insert(
        ...
        assert!(
            ...
            "a payment was auto-published for an order sharing its address"
        );
        assert_eq!(
            state.address_warning(&order.order.id),
            Some(AddressWarning::SharedWithAnotherOrder)
        );
        assert_eq!(
            state.address_warning(&twin.order.id),
            Some(AddressWarning::SharedWithAnotherOrder)
        );
    }
    ...
    /// An address contract's state as the bridge would publish it: a scanned
    /// watermark at `scanned_height`, plus whatever claims are given.
    ///
    /// The watermark is what orders two copies of the same contract, so it is
    /// named rather than defaulted: a test about a stale answer is a test
    /// about exactly this number.
    fn address_state_scanned_to(
        order: &AuthorizedOrder,
        scanned_height: u32,
        claims: Vec<freenet_bitcoin_common::SignedClaim>,
        ...
        let params = order.order.bitcoin_params();
        ...
        let mut all = vec![watermark];
        all.extend(claims);
        freenet_bitcoin_common::BitcoinAddressStateV1::from_claims(&params, all)
            .expect("the fixture's claims verify")
    }

    /// [`address_state_scanned_to`] at the tip the fixtures use.
    fn address_state_carrying(
        order: &AuthorizedOrder,
        claims: Vec<freenet_bitcoin_common::SignedClaim>,
    ) -> freenet_bitcoin_common::BitcoinAddressStateV1 {
    ...
    fn an_unsettled_order_asks_for_its_payment_address_again() {
        ...
        assert!(
            !state.due_address_rereads(0).1.is_empty(),
            "an unsettled order is a reason to look"
        );
        assert_eq!(
            tick_rereads(&mut state, 0),
            vec![id],
    ...
    fn a_settled_order_stops_being_asked_about() {
        ...
        assert!(
            !tick_rereads(&mut state, 0).is_empty(),
        ...
        assert!(
            tick_rereads(&mut state, 60_000).is_empty(),
            "a paid order is not asked about again"
        );
        let (wanted, due) = state.due_address_rereads(0);
        assert!(
    ...
    fn an_order_past_the_payable_window_stops_being_asked_about() {
        ...
        assert!(
            tick_rereads(&mut state, 10 * 60_000).is_empty(),
    ...
    fn the_same_address_is_not_asked_for_twice_in_a_minute() {
        ...
        assert_eq!(tick_rereads(&mut state, 0).len(), 1, "asked once");
        assert!(
            tick_rereads(&mut state, 1_000).is_empty(),
            "and not again a second later"
        );
        assert!(
            tick_rereads(&mut state, crate::address_reread::FIRST_RETRY_MS - 1).is_empty(),
            "nor a millisecond before the shortest wait"
        );
        // Asserted as a range rather than an instant because this order's
    ...
    /// **An address state that arrives is routed, and settles the order.**
    ///
    /// The payoff of the whole change, and the step no other test covered:
    /// an arriving state is routed to an address view by the
    /// `address_contract_network` map, so an answer to an id the re-read
    /// asked for but never registered would be tried as store state and
    /// thrown away. Asserting the map has an entry proves the entry, not the
    /// routing. This drives the real bytes through `on_contract_state`.
    #[test]
    fn a_re_read_answer_is_routed_and_settles_the_order() {
        ...
        let id = order
            ...
            .bitcoin_address_instance_id()
            .expect("the fixture names a build");
        assert_eq!(tick_rereads(&mut state, 0), vec![id], "the re-read asks");
        assert!(
            state.settled_orders(STORE).is_empty(),
            "and nothing settles until the answer arrives"
        );

        // The node answers, with the state the healed copy would carry.
        let answer = address_state_carrying(&order, claims);
        state.on_contract_state(
            id.to_vec(),
        ...
        assert!(
            state.bitcoin.addresses.contains_key(id.as_slice()),
            "the answer was read as address state rather than guessed at as a store"
        );
        assert!(
            state.settlements_submitted.contains(&order.order.id),
            "and arriving state settles the order, which is the point of asking"
        );
    }

    /// **A re-read that arrives short of the amount does not settle, and does
    /// not restart the clock.**
    ///
    /// Both halves matter. Publishing `Paid` on evidence that does not carry
    /// it is a state every peer refuses. And arriving state must not reset
    /// the spacing, or a node serving the same stale answer forever would
    /// hold the fastest cadence forever -- which is exactly the case the
    /// widening exists to bound.
    #[test]
    fn an_answer_that_does_not_settle_does_not_reset_the_wait() {
        let (order, _, tip) = a_paid_order();
        let (mut state, _) = buyer_after_acceptance(&order);
        let mut tip_view = tip_at(TIP_HEIGHT);
        tip_view.signed_tip = Some(tip);
        ...
        let answer = address_state_carrying(&order, an_underpayment(&order));
        state.on_contract_state(
            id.to_vec(),
            freenet_bitcoin_common::to_cbor(&answer).expect("cbor"),
        );
        ...
        assert!(
            tick_rereads(&mut state, 1_000).is_empty(),
            "and the answer did not put the address back at the front of the queue"
        );
    }

    /// **The SELLER's own unsettled order is asked about too.**
    ///
    /// The party that was actually stuck in the live incident. Only the
    /// seller can publish the settlement (`submit_order_by_id` requires the
    /// store to be theirs), so a fix that worked only on a buyer's tab would
    /// not have fixed the reported bug at all.
    #[test]
    fn a_sellers_own_unsettled_order_is_asked_about() {
        let (mut state, order) = seller_with_an_issued_invoice();
        let id = order
            .order
            .bitcoin_address_instance_id()
            .expect("the fixture names a build");
        ...
        assert_eq!(
            tick_rereads(&mut state, 0),
            vec![id],
            "the seller's own invoice is re-read"
        );
    }

    /// **The watch set is not narrowed to the re-read set.**
    ///
    /// The two share one walk, so the filter is the only thing keeping them
    /// different. `settled_orders` delegates its scoping to what the watch
    /// path subscribed to, so narrowing the watch set to "still unsettled"
    /// would quietly stop filling `bitcoin.addresses` for the very orders
    /// whose settlement is being published. Found by mutation: this is the
    /// change that CI did not previously notice.
    #[test]
    fn the_watch_set_still_covers_orders_the_re_read_skips() {
        let (order, claims, tip) = a_paid_order();
        let (mut state, _) = buyer_after_acceptance(&order);
        give_the_node_the_chain(&mut state, &order, claims, tip);
        let id = order
        ...
        // Move the order past the point where it is worth re-reading.
        let settled = state.settled_orders(STORE).pop().expect("settles");
        let store = state.browsing_stores.get_mut(STORE).expect("the store");
        for existing in store.orders.iter_mut() {
            if existing.order.id == settled.order.id {
        ...
        assert!(
            state.address_contracts_to_reread(STORE).is_empty(),
            "a paid order is not re-read"
        );
        assert!(
            state.address_contracts_to_watch(STORE).contains(&id),
            "but it is still watched, which is what settled_orders relies on"
        );
    }

    /// **A stale answer cannot erase what is already known.**
    ///
    ...
    fn a_stale_answer_cannot_erase_what_is_known() {
        ...
        // Asked for, which is what registers the id so an answer routes.
        assert_eq!(tick_rereads(&mut state, 0), vec![id], "asked");

        // The good answer lands first.
        let fresh = address_state_carrying(&order, claims);
        state.on_contract_state(
            id.to_vec(),
            freenet_bitcoin_common::to_cbor(&fresh).expect("cbor"),
        );
        let seen = state.bitcoin.addresses.get(id.as_slice()).expect("a view");
        let confirmed = seen.confirmed_sats;
        let claim_count = seen.claims.len();
        assert!(confirmed > 0, "the fixture pays the order");

        // Then a copy from before it, carrying nothing.
        let stale = address_state_scanned_to(&order, TIP_HEIGHT - 10, Vec::new());
        assert!(
            stale.scanned_to() < fresh.scanned_to(),
            "the fixture's stale copy really is behind"
        );
        state.on_contract_state(
            id.to_vec(),
        ...
        assert_eq!(after.claims.len(), claim_count, "claims are not erased");
        assert_eq!(
    ...
    /// **An order with no listing tag, for a conversation that asked for
    /// something, is refused.** An honest seller answering a request always
    /// writes one, so a missing tag fails closed rather than skipping the
    /// check.
    #[test]
    fn an_untagged_order_for_a_conversation_that_asked_is_refused() {
        let mut published = commitment(
            &seller_signing_key(),
            Some(anchor(TIP_HEIGHT)),
            OrderStatus::AwaitingPayment,
        );
        published.order.listing_tag = None;
        let published = resigned(published, &seller_signing_key());
        let (mut state, _) = buyer_after_acceptance(&published);
        let conversation = state.browsing_stores[STORE].conversations[0].clone();
        let request = conversation
    ...
    /// **A tag computed under another conversation's key does not match.**
    #[test]
    fn a_listing_tag_from_another_conversation_is_refused() {
        let (_, stranger) = buyer_conversation();
        let mut published = commitment(
            &seller_signing_key(),
            Some(anchor(TIP_HEIGHT)),
            OrderStatus::AwaitingPayment,
        );
        published.order.listing_tag = Some(stranger.listing_tag(&widget()));
        let published = resigned(published, &seller_signing_key());
        let (mut state, _) = buyer_after_acceptance(&published);
        let conversation = state.browsing_stores[STORE].conversations[0].clone();
        let request = conversation
    ...
    /// starts over.** Requests sent to an inbox the bridge has left would
    /// otherwise hold the Ghost Key's places for ever.
    #[test]
    fn a_new_inbox_generation_replaces_what_was_sent_to_the_old() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        state.bitcoin.inbox = None;
        state.register_inbox_contract(inbox::bridge(), [0x38; 32]);
        let key = state.bitcoin.inbox.as_ref().expect("tracked").contract_key;
        assert_eq!(key, inbox::inbox_key());
        serve_inbox(&mut state, &inbox::open_inbox());
        assert!(state
            .bitcoin
            .inbox
        ...
        assert!(
            state
                ...
                .state
                .is_some(),
            "the same generation keeps what it knows"
        );

        state.register_inbox_contract(inbox::bridge(), [0x39; 32]);
        let tracker = state.bitcoin.inbox.as_ref().expect("tracked");
        assert_ne!(tracker.contract_key, key);
        assert!(tracker.state.is_none(), "a new generation starts over");
    }

    /// **A buyer's tab never touches the inbox**, and a seller's fetches it
    /// once, again only after a pause if nothing came back.
    #[test]
    fn the_inbox_is_fetched_only_for_a_node_with_orders_to_watch() {
        let gk = inbox::authority().mint();
        let mut buyer = a_seller_selling(vec![], gk.id().0);
        assert_eq!(buyer.queue_due_watch_requests(1_000), WatchWork::default());

        let mut seller = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        let work = seller.queue_due_watch_requests(1_000);
        assert_eq!(work.fetch, Some(*inbox::inbox_key().id()));
        assert!(work.sign.is_empty(), "nothing to date an entry against yet");
        assert_eq!(
            seller.queue_due_watch_requests(2_000).fetch,
            None,
            "not asked again at once"
        );
        assert!(
            seller
                .queue_due_watch_requests(1_000 + crate::bitcoin_inbox::INBOX_FETCH_RETRY_MS)
                .fetch
                .is_some(),
            "asked again when nothing came back"
        );
    }

    /// **Nothing is queued while the seller is signing something of their
    /// own**, because a refusal names no request.
    #[test]
    fn no_watch_request_is_queued_while_the_seller_is_signing() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        state.request_any_access_in_flight = true;
        serve_inbox(&mut state, &inbox::open_inbox());
    ...
    /// **A refusal that can only be about a watch request clears nothing of
    /// the seller's, tells them once, and holds off.**
    #[test]
    fn a_refused_watch_signature_is_contained() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        ...
        assert!(
            queued_watch_requests(&state).is_empty(),
            "held off rather than asked again at once"
        );
        state.on_ghostkey_response(ghostkey_common::GhostkeyResponse::PermissionDenied {
            fingerprint: "seller-fp".into(),
            requestor: harvest_common::expected_harvest_requestor(),
        });
        let about_watching = state
            .notifications
            .iter()
            .filter(|n| n.contains("Bitcoin bridge"))
            .count();
        assert_eq!(
            about_watching, 1,
    ...
    fn a_refusal_with_the_sellers_own_signature_outstanding_is_theirs() {
        ...
        assert!(
            ...
            "the ordinary handling ran"
        );
        assert!(
            state.bitcoin.watch_requests_stopped.is_empty(),
            "and it was not counted against the watch requests"
        );
    }

    /// **A signature request that never comes back is given up**, so it does
    /// not hold one of the Ghost Key's two places for the rest of the session.
    #[test]
    fn a_watch_signature_that_never_returns_is_given_up() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        let mut pending = inbox_pending_for(&gk);
        pending.queued_at_ms = 1_000;
        state
            .pending_signatures
            .push_back(PendingSignature::InboxEntry(Box::new(pending)));
        state.queue_due_watch_requests(1_000 + crate::bitcoin_inbox::SIGNATURE_TIMEOUT_MS - 1);
        assert_eq!(queued_watch_requests(&state).len(), 1, "still waiting");
        state.queue_due_watch_requests(1_000 + crate::bitcoin_inbox::SIGNATURE_TIMEOUT_MS);
        assert!(queued_watch_requests(&state).is_empty());
    }

    /// **A signature for an inbox this tab has left is not sent.**
    #[test]
    fn a_watch_request_for_a_replaced_inbox_is_not_sent() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        serve_inbox(&mut state, &inbox::open_inbox());
        let queued = queued_watch_requests(&state).remove(0);
        ...
        assert!(state
            .on_inbox_entry_signed(queued, gk.pem.clone(), scoped_payload, signature, 1)
            .is_none());
    }

    #[test]
    fn an_order_with_no_anchor_is_not_watched() {
        let gk = inbox::authority().mint();
        let mut order = commitment(&seller_signing_key(), None, OrderStatus::AwaitingPayment);
        order.order.trusted_bridges.push(inbox::bridge());
        let order = resigned(order, &seller_signing_key());
    ...
    fn every_refusal_of_a_watch_signature_is_contained() {
        ...
        for refusal in refusals {
            ...
            assert!(
                state.bitcoin.watch_requests_stopped.contains("seller-fp"),
                "{what}: that key is not asked again"
            );
    ...
    fn a_refusal_naming_another_key_is_handled_as_before() {
        ...
        assert!(
            state.bitcoin.watch_requests_stopped.is_empty(),
            "the seller's key is not stopped for another key's refusal"
        );
    ...
    /// **After a refusal the key is not asked again this session**, so a
    /// revoked grant raises the vault's dialog at most once, and approving it
    /// to make it stop is never needed.
    #[test]
    fn a_refused_key_is_not_asked_again_this_session() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        serve_inbox(&mut state, &inbox::open_inbox());
        state.on_ghostkey_response(ghostkey_common::GhostkeyResponse::PermissionDenied {
        ...
        for hours in 1..30 {
            let later = now_ms() + hours * 60 * 60 * 1000;
            let mut fresh = inbox::open_inbox();
            inbox::raise_floor(&mut fresh, inbox::FLOOR + hours as u32 * 6);
            state
                .bitcoin
                .inbox
                .as_mut()
                .expect("tracked")
                .on_state(fresh, later);
            assert!(state.queue_due_watch_requests(later).sign.is_empty());
        }
        assert_eq!(state.notifications.len(), 1, "told once");
    }

    /// **A signature that never returns, or one that cannot be sent, stops
    /// its key the same way.**
    #[test]
    fn a_silent_or_unusable_signature_stops_its_key() {
        let gk = inbox::authority().mint();
        let mut silent = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        serve_inbox(&mut silent, &inbox::open_inbox());
        let later = now_ms() + crate::bitcoin_inbox::SIGNATURE_TIMEOUT_MS;
        silent.queue_due_watch_requests(later);
        assert!(queued_watch_requests(&silent).is_empty());
        assert!(silent.bitcoin.watch_requests_stopped.contains("seller-fp"));
        assert_eq!(silent.notifications.len(), 1);

        let stranger = inbox::authority().mint();
        let mut wrong = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        serve_inbox(&mut wrong, &inbox::open_inbox());
        let queued = queued_watch_requests(&wrong).remove(0);
        for _ in 0..2 {
            let (scoped_payload, signature) =
                inbox::sign_result(&stranger, queued.signing_payload.clone());
            wrong.on_inbox_entry_signed(
                queued.clone(),
                stranger.pem.clone(),
                scoped_payload,
                signature,
        ...
        assert!(wrong.bitcoin.watch_requests_stopped.contains("seller-fp"));
        assert_eq!(wrong.notifications.len(), 1, "told once");
    }

    /// **Requests are queued for one Ghost Key at a time**, so a refusal that
    /// names no key stops only the key it was about.
    #[test]
    fn requests_go_out_one_ghost_key_at_a_time() {
        let first = inbox::authority().mint();
        let second = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], first.id().0);
        // A second store, under a second listed key.
        const OTHER: &[u8] = &[8u8; 32];
        state.my_stores.insert(
            "second-fp".to_string(),
            vec![StoreRegistration {
                store_contract_id: OTHER.to_vec(),
                reputation_contract_id: vec![12u8; 32],
                mailbox_contract_id: vec![13u8; 32],
                store_contract_key: None,
            }],
        ...
        state.ghostkeys.push(ghostkey_common::GhostKeyInfo {
            fingerprint: "second-fp".to_string(),
            label: None,
            notary_info: String::new(),
            verifying_key_bytes: Some(second.id().0.to_vec()),
            backed_up: false,
        });
        state.begin_browsing(OTHER.to_vec());
        let mut theirs = an_order_naming_the_test_bridge(4);
        theirs.order.seller_fingerprint = "second-fp".to_string();
        ...
        assert_eq!(
            state
                ...
                .collect::<Vec<_>>(),
            vec![&first_fp],
            "an unnamed refusal stops only the key that was asked"
        );
        state.send_due_watch_requests();
        let next = queued_watch_requests(&state);
        assert_eq!(next.len(), 1);
        assert_ne!(next[0].fingerprint, first_fp, "the other key goes next");
    }

    /// **The hold restarts whenever the seller's signing clears**, so two
    /// separate actions do not add up to the limit.
    #[test]
    fn the_hold_restarts_between_separate_actions_of_the_seller() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        let start = now_ms();
        state
            ...
            .expect("tracked")
            .on_state(inbox::open_inbox(), start);
        let limit = crate::bitcoin_inbox::MAX_HOLD_FOR_USER_MS;
        ...
        state
            ...
            .on_state(fresh, at);
        assert!(
            state.queue_due_watch_requests(at).sign.is_empty(),
            "held from the second action's start, not the first's"
        );
    }

    #[test]
    fn the_watch_check_acts_while_a_request_is_outstanding() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![], gk.id().0);
        assert!(!state.watch_check_could_act());
        state
            .pending_signatures
            .push_back(PendingSignature::InboxEntry(Box::new(inbox_pending_for(
                &gk,
            ))));
        assert!(state.watch_check_could_act(), "it may have timed out");
    }

    #[test]
    fn a_withdrawn_inbox_is_told_to_the_seller() {
        let gk = inbox::authority().mint();
        let mut state = a_seller_selling(vec![an_order_naming_the_test_bridge(3)], gk.id().0);
        state.withdraw_inbox();
